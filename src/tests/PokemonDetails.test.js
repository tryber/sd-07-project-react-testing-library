...
describe('Testando o arquivo PokemonDetails.js, requisito 7', () => {
  it('Teste se as informações detalhadas do Pokémon selecionado são mostradas na tela.', () => {
    ...
    const paragraphy = getByText(/The flame on its tail shows the strength of its life force./i);
  ...
  it('Teste se existe na página uma seção com os mapas contendo as localizações do pokémon.', () => {
    const { getByRole, getByText, getByTestId, getAllByAltText } = renderWithRouter(<App />);
    const next = getByTestId('next-pokemon');
    fireEvent.click(next);
    const details = getByRole('link', { name: 'More details' });
    expect(details).toBeInTheDocument();
