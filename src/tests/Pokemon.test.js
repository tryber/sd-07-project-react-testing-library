...
describe('Testando o arquivo Pokemon.js', () => {
  ...
  it('Teste se é renderizado um card com as informações de determinado pokémon.', () => {
    const { getByText, getByRole, queryAllByRole, getByTestId } = renderWithRouter(<App />);
  ...
  it('Teste se o card do Pokémon indicado na Pokédex contém um link de navegação para exibir detalhes deste Pokémon. O link deve possuir a URL /pokemons/<id>, onde <id> é o id do Pokémon exibido;', () => {
    ...
    const pokeName = getByTestId('pokemon-name').innerHTML;
    const pokemon = data.find(pokemon => pokemon.name === pokeName);
  ...
  it('Teste se existe um ícone de estrela nos Pokémons favoritados.', () => {
    const { getByText, getByAltText, getByTestId, history, getByLabelText } = renderWithRouter(<App />);
  ...
  it('Matando Mutantes 2.0', () => {
    ...
    renderWithRouter(<Pokemon pokemon={data[0]} showDetailsLink={false} isFavorite={pokeFav} />);
