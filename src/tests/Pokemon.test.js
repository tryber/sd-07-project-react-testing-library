...
const mockPokemons = [
  {
    ...
    summary: 'This intelligent ... to make them tender enough to eat.',
  },
  {
    id: 4,
    name: 'Charmander',
  ...
  },
  {
    ...
    summary: 'For protection, it releases ... enemies.',
  },
  {
    id: 23,
    name: 'Ekans',
  ...
  },
  {
    ...
    summary: 'Apparently, ... a strong desire to see it.',
  },
  {
    id: 78,
    name: 'Rapidash',
    ...
    summary: 'At full gallop, ... incredibly fast.',
  },
  {
    id: 143,
    name: 'Snorlax',
...
describe('Testa Pokemon.js', () => {
  it('testa se é renderizado um card com as informações de determinado pokémon', () => {
    const { getByTestId, getByRole } = renderWithRouter(
      <Pokemon pokemon={ mockPokemons[0] } isFavorite={ false } />,
    );
    const pokemonName = getByTestId('pokemon-name');
    expect(pokemonName).toHaveTextContent(mockPokemons[0].name);

    const pokemonType = getByTestId('pokemonType');
    expect(pokemonType).toHaveTextContent(mockPokemons[0].type);

    const { value, measurementUnit } = mockPokemons[0].averageWeight;

    const pokeAverage = getByTestId('pokemon-weight');
    expect(pokeAverage).toHaveTextContent(`Average weight: ${value} ${measurementUnit}`);

    const { image, name } = mockPokemons[0];
    const pokeAlternative = `${name} sprite`;
    const pokeImage = getByRole('img');
    expect(pokeImage.src).toBe(image);
    expect(pokeImage.alt).toBe(pokeAlternative);
  });
  it('testa se exibe um link para os detalhes do pokémon na tela', () => {
    const { getByRole } = renderWithRouter(
      <Pokemon pokemon={ mockPokemons[0] } isFavorite={ false } />,
    );

    const { id } = mockPokemons[0];
    const details = getByRole('link', { value: 'More details' });
    expect(details).toBeInTheDocument();
    expect(details.href).toBe(`http://localhost/pokemons/${id}`);
  });
  it('testa o redirecionamento da página de detalhes do pokémon', () => {
