...
describe('Favorite Pokemons - Teste de conteúdo', () => {
  it('Deve exibir "No favorite pokemon found" quando não há fav', () => {
    const { getByText } = renderWithRouter(<FavoritePokemons pokemons={ [] } />);
    const notFound = getByText('No favorite pokemon found');
    expect(notFound).toBeInTheDocument();
  });

  it('Deve renderizar os primeiros três pokemons', () => {
    const pokemons = [
      {
        id: 25,
        name: 'Pikachu',
    ...
    const { getAllByTestId } = renderWithRouter(<FavoritePokemons
      pokemons={ pokemons }
    />);
    const quantPokemons = getAllByTestId('pokemon-name');
    expect(quantPokemons.length).toBe(pokemons.length);
  });

  // it('Não deve exibir nenhum card se não há favoritos', () => {
  //   renderWithRouter(<FavoritePokemons pokemons={ [] } />);
  //   expect(notFound).toHaveBeenCalled(1);
  // });
});
