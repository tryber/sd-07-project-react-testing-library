...
test('se é exibido todos os cards de pokemons favoritados', () => {
  renderWithRouter(<FavoritePokemons pokemons={ pokemons } />);
  const divs = document.getElementsByClassName('favorite-pokemon');
  const nmagic = 2;
  expect(divs.length).toBe(nmagic);
});

test('se nenhum card de pokemon é exibido, se ele não estiver favoritado', () => {
